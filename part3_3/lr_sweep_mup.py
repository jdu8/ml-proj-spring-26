"""
LR sweep for µP Tiny model.

The optimal LR found here transfers to all larger models via the
manual width scaling in train_mup.py (mup_lr = lr * BASE_WIDTH / n_embd).
...
def main():
    ...
    print(f"µP LR sweep on config='{args.config}'  ({args.max_iters} steps each)")
    ...
    for lr in args.lrs:
        ...
        print(f"\n{'='*60}")
        print(f"  µP nominal LR = {lr:.2e}  →  {run_dir}")
        print(f"{'='*60}")

        # use a fixed total_steps override so all sweep runs are the same length
        cmd = [
    ...
    print(f"\nBest µP nominal LR: {best['lr']:.2e}  (val loss {best['val_loss']:.4f})")
    print(f"Summary → {sp}")
