"""
Train a GPT-µP language model on SVG data for exactly 1 epoch.

Mirrors part2/train.py but uses GPTMuP + MuAdamW for µP-compliant training.
The LR tuned on the Tiny model transfers to all larger models via MuAdamW's
automatic per-layer LR scaling (base_width / layer_width).

Quickstart:
    # point at part2's pre-built data directory
    python train_mup.py --config tiny         --lr 3e-3 --data_dir ../part2/data --out_dir out/tiny
    python train_mup.py --config xl_wide      --lr 3e-3 --data_dir ../part2/data --out_dir out/xl_wide

Output (in --out_dir):
    ckpt.pt       — best checkpoint (by val loss)
    log.csv       — per-step metrics
    results.json  — final summary used by scaling_plot_mup.py
"""
import argparse
import csv
import json
...
def parse_args():
    ...
    p.add_argument("--config",       default="tiny",  help="Model size (wide series recommended)")
    p.add_argument("--lr",           type=float, default=3e-3)
    p.add_argument("--min_lr_frac",  type=float, default=0.1, help="min_lr = lr * min_lr_frac")
    p.add_argument("--batch_size",   type=int,   default=32,  help="Micro-batch size (sequences/step)")
...
def main():
    ...
    # make_mup_model calls set_base_shapes internally — must do this before .to(device)
    model = make_mup_model(gpt_cfg).to(device)
    n_params       = model.count_params()
    n_params_total = model.count_params(non_embedding=False)

    print(f"\n{'─'*55}")
    print(f"  Model : {args.config}  [µP]")
    ...
    width_mult = gpt_cfg.n_embd / 128  # ratio to Tiny base
    print(f"  Width multiplier : {width_mult:.2f}x  (effective LR ≈ {args.lr/width_mult:.2e})")
    ...
    # ── optimizer (MuAdamW handles per-layer LR scaling automatically) ─────────
    ...
    for step in range(total_steps):
        ...
        # update base LR; MuAdamW scales this per-layer by base_width/layer_width
