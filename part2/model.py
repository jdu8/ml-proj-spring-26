"""
Decoder-only transformer (GPT), adapted from nanoGPT (github.com/karpathy/nanoGPT).

Changes from the original:
  - GPTConfig has an explicit n_ff field instead of hardcoding 4 * n_embd
  - Removed from_pretrained and crop_block_size (not needed)
  - vocab_size defaults to 4096 (our BPE vocabulary)
  - Added count_params(), top_p support in generate()
"""
import math
import inspect
...
class LayerNorm(nn.Module):
    """LayerNorm with optional bias (nn.LayerNorm always has bias)."""
...
class GPTConfig:
    ...
    n_head: int = 4
    n_embd: int = 128
    n_ff: int = 512      # feedforward hidden dim; typically 4 * n_embd
    dropout: float = 0.0
    bias: bool = False   # no bias → fewer params, generally same or better perf
    ...
    def _init_weights(self):
        for module in self.modules():
            if isinstance(module, nn.Linear):
                ...
                nn.init.normal_(module.weight, mean=0.0, std=0.02)
        # scaled init for residual projections, as in GPT-2 paper
        for pn, p in self.named_parameters():
            if pn.endswith("c_proj.weight"):
                nn.init.normal_(p, mean=0.0, std=0.02 / math.sqrt(2 * self.config.n_layer))

    def count_params(self, non_embedding=True):
        """Total parameters, optionally excluding positional embeddings (Kaplan et al. convention)."""
